...
- **cookieName** (string) _(optional)_: Filter requests by an exact cookie name. By default this returns only responses that set/update this cookie. Combine with cookieRelation="sends" to find requests carrying the cookie, or cookieRelation="all" to show both sends and updates.
- **cookieRelation** (enum: "updates", "sends", "all") _(optional)_: How cookieName should match requests: "updates" matches response Set-Cookie entries for that cookie, "sends" matches request Cookie headers carrying that cookie, and "all" matches either relation. Defaults to "updates".
